...
<div class="position-relative">
    ...
    <div v-cloak id="input-controls" class="controls" v-if="total_devices > 0" :class="{'fixed': overlayControls}">
        ...
        <button
            ...
            :title="'<?php echo addslashes(tr('Select all')); ?>' + ' (' + total_inputs + ')'">
            <svg class="icon">
    ...
    <div id="app" v-cloak class="mt-4">
        ...
        <template v-if="loaded">
            <template v-if="total_devices > 0">
                <div
                    class="node accordion line-height-expanded shadow-sm rounded mb-3 bg-white"
                    v-for="(device,nodeid) in devices"
                    :class="{'select-mode': selectMode}">
                    <div
                        @click="toggleCollapse($event, nodeid)"
                        class="node-info accordion-toggle thead"
                        :style="{ '--status-color': device.time_color }"
                        :data-node="nodeid"
                        :data-target="'#collapse_' + nodeid">
                        ...
                        <div class="select text-center has-indicator" data-col="B">
                            ...
                            <input
                                ...
                                type="checkbox"
                                class="checkbox-lg"
                                :checked="isFullySelected(device)"
                                :title="'<?php echo addslashes(tr("Select all %s inputs")); ?>'.replace('%s',getDeviceInputIds(device).length)">
                        </div>

                        <h5 class="name text-nowrap" data-col="A" :style="{width:col.A+'px'}">
                            <span>{{ nodeid }}
                                <small class="position-absolute ml-1" v-if="getDeviceSelectedInputids(device).length > 0">
                                    ({{ getDeviceSelectedInputids(device).length }})
                                </small>
                            </span>
                        </h5>

                        <span class="description text-nowrap" data-col="G" :style="{width:col.G+'px'}">
                            {{device.description}}
                        </span>
                        ...
                        <div class="buttons pull-right">
                            ...
                            <div
                                ...
                                data-col="E"
                                :style="{width:col.E+'px', color:device.time_color}">
                                {{ device.time_value }}
                            </div>
                            <a
                                @click.prevent.stop="show_device_key(device)"
                                href="#"
                                class="device-key text-center"
                                data-col="D"
                                :style="{width:col.D+'px'}"
                            ...
                            <a
                                ...
                                href="#"
                                class="device-configure text-center"
                                data-col="C"
                    ...
                    <div
                        :id="'collapse_' + nodeid"
                        class="node-inputs collapse tbody"
                        :class="{in: collapsed.indexOf(nodeid) === -1}"
                        :data-node="nodeid">

                        <div
                            @click="toggleSelected($event, input.id)"
                            class="node-input py-2"
                            :id="input.id"
                            v-for="(input,index) in device.inputs"
                            :style="{ '--status-color': input.time_color }"
                            :class="[{'selected': selected.indexOf(input.id) > -1}]">

                            <div class="select text-center" data-col="B">
                                <input class="input-select" type="checkbox" :value="input.id" v-model="selected">
                            </div>

                            <div class="name text-nowrap" data-col="A" :style="{width:col.A+'px'}">
                                {{ input.name }}
                            </div>

                            <div class="description text-nowrap" data-col="G" :style="{width:col.G+'px'}">
                                {{ input.description }}
                            </div>
                            ...
                            <div class="buttons pull-right">
                                ...
                                <span
                                    @click.stop
                                    class="time text-center break-all"
                                    data-col="E"
                                    :style="{width:col.E+'px', height:col_h.E, color:input.time_color}">
                                    {{ input.time_value }}
                                </span>
                                ...
                                <a
                                    @click.prevent.stop="showInputConfigure(input.id)"
                                    class="configure text-center cursor-pointer"
                                    data-col="C"
                                    :style="{width:col.C+'px'}"
                                    :id="input.id"
